...
from ..components.result_card import render_result_card, score_bar, highlight_query_terms
...
def render_advanced_filters() -> Dict[str, Any]:
    ...
    return {
        ...
        "content_type": content_type_filter if content_type_filter != "All Types" else None,
...
def render_search_result_card(rank: int, result: Dict, query: str) -> None:
    ...
    with st.container(border=True):
        ...
        col1, col2 = st.columns([3, 1])
        with col1:
            st.markdown(f"**{content_icon} {title[:60]}{'...' if len(title) > 60 else ''}**")
        with col2:
            st.caption(f"📅 {date}")
        
        st.caption(f"Chunk {chunk_index} • Source: `{source_id}` • Type: {content_type}")
        ...
        with btn_col3:
            # Show link button only for SRT/video content that looks like YouTube IDs
            if content_type == "srt" and source_id and source_id != "N/A" and len(source_id) == 11:
                st.link_button(
                    "▶️ Video",
